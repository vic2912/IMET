...
import {
  ...
  AccordionDetails, useMediaQuery, Button, Stack
} from '@mui/material';
...
import { Booking, PersonDetails } from '../types/booking';
import {
        ...
        <Stack spacing={1}>
          {daysInMonth.map((day) => {
            ...
            return (
              <Box
                key={key}
                sx={{ border: '1px solid #ccc', p: 1, cursor: 'pointer', backgroundColor: totalPeople >= 1 ? '#c8e6c9' : dayBookings.length ? '#e3f2fd' : 'white' }}
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 1 }}>
          {daysInMonth.map((day) => {
            ...
            return (
              <Box
                key={key}
                sx={{ border: '1px solid #ccc', p: 1, minHeight: { xs: 60, sm: 80 }, cursor: 'pointer', backgroundColor: totalPeople >= 1 ? '#c8e6c9' : dayBookings.length ? '#e3f2fd' : 'white' }}
                onClick={() => setSelectedDate(day)}
              >
                <Typography variant="subtitle2">
